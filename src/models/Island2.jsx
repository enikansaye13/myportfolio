...
import islandScene from "../assets/3d/cloud_station.glb";

const Island = ({ isRotating, setIsRotating, setCurrentStage, ...props }) => {
  const islandRef = useRef();

  const { gl, viewport } = useThree(); // this give access to js three renderer and viewport
  const { nodes, materials } = useGLTF(islandScene);
  const lastX = useRef(0); //ref to know the last mouse position
  // lastX means horizontal position
  const rotationSpeed = useRef(0); //Rotation speed
  const dampingFactor = 0.95; //how fast does it move when you scroll and how does it continue moving after what
  ...
  const handlePointerDown = (e) => {
    ...
    //figure out if its a touch event or a mouse event
    const clientX = e.touches 
    ? e.touches[0].clientX : e.clientX;
    lastX.current = clientX; //Store  last positio
  };
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating) {
      ...
      // island rotation base on the mouse
      islandRef.current.rotation.y += delta * 0.01 * Math.PI;
  
      // reference for last client exposition
  ...
  //ability to move with the keyboard
  ...
  useFrame(() => {
    if(!isRotating){
      ...
      islandRef.current.rotation.y += rotationSpeed.current;
        ...
        //set the current stage base on the island's orientation
        switch (true) {
  ...
  // taking all the handle function to effect
  useEffect(() => {
    ...
    return () => {
      //remove from the canva when we exit the page
